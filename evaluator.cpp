...
void trimTrailingWhitespace(std::string &line){while(!line.empty()&&(line.back()==' '||line.back()=='\t'||line.back()=='\r')) line.pop_back();}
...
ofstream extlog;
bool customJudge(const string &input,const string &output,const string &answer,const string &checker,const int testid)
{
	string checker_output="temp\\checker.exe";
	// 运行 checker，testlib 格式
	string command=checker_output+" "+input+" "+output+" "+answer+" > temp\\checker_output.txt 2>&1";
	if(system(command.c_str()))
	{
		ifstream fin("temp\\checker_output.txt");
		extlog<<"On Test "<<testid<<endl;
		string line;
		while(getline(fin,line)) extlog<<line<<endl;
		return 0;
	}
	ifstream fin("temp\\checker_output.txt");
	extlog<<"On Test "<<testid<<endl;
...
// By Federico,All rights reserved
JudgeInfo judgeProgram(const string &filename,const string &input,const string &output,const string &answer,const string &spj,int testid)
{
	JudgeInfo result;
	...
	if(success)
	{
		...
		{
			// 根据实际需求，调用对应对比函数
			bool differ=spj=="none"?compareFiles(output,answer):customJudge(input,output,answer,checker_file,testid);
			// 注意：本示例里函数返回 1 代表“相同”，
			// 你可根据自己的实现对逻辑进行调整
			if(differ) result.result=ACCEPTED;
			else result.result=WRONG_ANSWER;
		}
	}
	else result.result=SYSTEM_ERROR;
	return result;
}
// 替换 judge 函数中的评测流程，调用新函数 judgeProgram
void judge(const std::string &input_file,const std::string &output_file,const std::string &executable,const std::string &file_io,const std::string &time_limit_str,const std::string &memory_limit_str,const std::string &special_judge,const std::string &checker_file,std::ofstream &evaluator_log,int testid)
{
	...
	JudgeInfo info=judgeProgram(executable,input_file,user_output_file,output_file,checker_file,testid);
	...
	if(info.result==ACCEPTED||info.result==WRONG_ANSWER)
	{
		...
		if(outCheck.is_open())
		{
			...
			if(size>100LL*1024*1024) info.result=OUTPUT_LIMIT_EXCEEDED;
		}
	}
	// 映射回原判题系统需要的字符串
	...
	// 将评测结果记录到日志
	// 时间以毫秒计；内存以 KB 计
	evaluator_log<<"TIME "<<info.time<<" MEMORY "<<(info.memory/1024)<<" VERDICT "<<verdict<<std::endl;
...
signed main(int argc,char* argv[])
{
	...
	if(argc!=19)
	{
		...
		cerr<<"Usage: evaluator --problem [ID] --submission-id [ID] --file-io [0/1] --input-file [stdin/input_filename] --output-file [stdout/output_filename] --time-limit [time_limit] --memory-limit [memory_limit] --special-judge [0/1] --checker-file [none/special_judge_filename]"<<endl;
		return 1;
	}
	...
	extlog.open("log\\extra-"+submission_id+".log");
	...
	{
		for(auto [i,o]:files) evaluator_log<<"TIME 0 MEMORY 0 VERDICT CE"<<endl;
		return 1;
	}
	int TESTID=0;
	if(checker_file!="none")
	{
		...
		string checker_filename="problems\\data\\"+problem_id+"\\"+checker_file;
		string checker_output="temp\\checker.exe";
		string command="g++ -O2 -std=c++17 -o "+checker_output+" "+checker_filename;
		if(system(command.c_str()))
		{
			for(auto [i,o]:files) evaluator_log<<"TIME 0 MEMORY 0 VERDICT UKE"<<endl;
			return 0;
		}
	}
	...
	for(auto [i,o]:files)
	{
		...
		judge(input_file,output_file,random_exe,Arg["--file-io"],time_limit,memory_limit,Arg["--special-judge"],checker_file,evaluator_log,++TESTID);
	}
	return 0;
}
